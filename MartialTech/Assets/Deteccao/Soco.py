...
def printPunch(p1, p2, p3, img=None, color=(255, 0, 255), scale=5, angle2=0):
    """
...
def distanceXaxis(p1, p2, mindist=50):
    x1, y1 = p1
    x2, y2 = p2
    distx = abs(x2 - x1)
    if distx > mindist:
...
def ehsoco(img, lmList, detector):
    ...
    if verifySocoDir and isAngleArmHipDir:
        if absoluteDistanceXY(p13, p15) and distanceXaxis(p13, p15):
            ehsimdir = True
    ...
    if verifySocoEsq and isAngleArmHipEsq:
        if absoluteDistanceXY(p14, p16) and distanceXaxis(p13, p15):
            ehsimesq = True
